...
const YAML = join(import.meta.dirname, '..', '..', '..', 'coolify-openapi-v4.x.yaml')
const lines = readFileSync(YAML, 'utf8').split('\n')

/**
 * Raw spec lines, for the blocks this extractor deliberately does not model —
 * quoted paths such as `'/applications/{uuid}/envs':`, which specs scan directly.
 */
export const specLines: readonly string[] = lines
...
function propsAt(
  ...
  endLine: number,
  indent: number,
): Record<string, SpecProp> {
  const out: Record<string, SpecProp> = {}
  ...
  for (let i = startLine; i < endLine; i++) {
    ...
    if (current && ind > indent) {
      ...
      if (t && entry.type === null) entry.type = t[1]!
      const n = line.trim().match(/^nullable:\s*(true|false)/)
      if (n) entry.nullable = n[1] === 'true'
      const e = line.trim().match(/^enum:\s*\[(.*)\]/)
      if (e) {
        entry.enum = e[1]!.split(',').map((s) => s.trim().replace(/^'|'$/g, ''))
      }
    }
  }
  return out
...
function blockEnd(start: number, indent: number): number {
  for (let i = start + 1; i < lines.length; i++) {
    if (indentOf(lines[i]!) === indent && lines[i]!.trim().endsWith(':')) return i
  }
  return lines.length
}

/** Request schema (allowed properties + required) for a given path. */
export function createSchema(path: string): {
  required: string[]
 ...
 */
export function queryParams(path: string): Record<string, SpecParam> {
  ...
  const out: Record<string, SpecParam> = {}
  let current: SpecParam | null = null
...
export function componentSchema(name: string): Record<string, SpecProp> {
  const start = findLine(new RegExp(`^    ${name}:\\s*$`))
  if (start < 0) throw new Error(`component not found: ${name}`)
  const end = blockEnd(start, 4)
